...
kubectl delete mdbs rssearch --force &
kubectl patch mdbs rssearch -p '{"metadata":{"finalizers":null}}' --type=merge
...
kubectl delete pvc data-rssearch-search-0-0 --grace-period=0 --force &
sleep 20
kubectl get pvc -o name | xargs kubectl patch pvc -p '{"metadata":{"finalizers":null}}' --type=merge &
#kubectl patch pvc data-rssearch-search-0-0 -p '{"metadata":{"finalizers":null}}' --type=merge
