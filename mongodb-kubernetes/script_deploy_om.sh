...
echo "Rollout MCK1.12.0"
kubectl apply -f mongodb-init.yaml
kubectl config set-context $(kubectl config current-context) --namespace=mongodb
kubectl apply -f mongodb-storageclass-efs.yaml
kubectl apply -f crds1.12.0.yaml
kubectl apply -f mongodb-kubernetes1.12.0.yaml
...
kubectl create secret tls poc-pocbrace-cert --cert=tls/RS-pocbrace.crt --key=tls/RS-test-server.key --dry-run=client -o yaml | kubectl apply -f -
kubectl create secret tls poc-pocbrace-agent-certs --cert=tls/RS-rspbagent.crt --key=tls/RS-test-server.key --dry-run=client -o yaml | kubectl apply -f -
kubectl create secret tls poc-pocbrace-clusterfile --cert=tls/RS-pocbrace.crt --key=tls/RS-test-server.key --dry-run=client -o yaml | kubectl apply -f -
