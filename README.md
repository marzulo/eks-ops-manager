# eks-ops-manager
Deploy Ops Manager and MongoDB using MCK 1.12.0 on AWS EKS with Terraform
...
Finally, you can apply the MongoDB Search Node with a Replica Set using MongoDB Enterprise FCV 8.3:

```./script_deploy_search.sh <AWS_REGION> <EKS_CLUSTER_NAME> <PROFILE>```
...
#### Create Auto-embeddings Voyage AI API keys

1. Go to your MongoDB Cloud account and [create 2 Voyage AI API Keys](https://www.mongodb.com/docs/voyageai/management/api-keys/). 1 for Index Creation and another 1 for Query.

2. Update the `mongodb-search.yaml` with the API Key created, in the Secret `voyage-api-keys`.
